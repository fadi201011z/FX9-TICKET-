...
exports.data = new discord_js_1.SlashCommandBuilder()
    ...
    .addSubcommand((s) => s.setName("panel_channel").setDescription("📣 تعيين قناة البنل")
    .addChannelOption((o) => o.setName("channel").setDescription("القناة").addChannelTypes(discord_js_1.ChannelType.GuildText).setRequired(true))
    .addBooleanOption((o) => o.setName("send_now").setDescription("إرسال البنل فيها الآن؟").setRequired(false)))
    .addSubcommand((s) => s.setName("ticket_category").setDescription("📁 فئة التكتات (قناة العضو)")
    .addChannelOption((o) => o.setName("category").setDescription("الفئة").addChannelTypes(discord_js_1.ChannelType.GuildCategory).setRequired(true)))
    .addSubcommand((s) => s.setName("admin_category").setDescription("🔐 فئة قنوات الإدارة (الريلاي)")
    .addChannelOption((o) => o.setName("category").setDescription("فئة مخصصة للإدارة فقط").addChannelTypes(discord_js_1.ChannelType.GuildCategory).setRequired(false))
    .addBooleanOption((o) => o.setName("disable").setDescription("تعطيل نظام القنوات المزدوجة").setRequired(false)))
    .addSubcommand((s) => s.setName("support_role").setDescription("🛡️ إدارة رتب فريق الدعم (متعددة)")
    .addStringOption((o) => o.setName("action").setDescription("الإجراء").setRequired(true)
    .addChoices({ name: "➕ إضافة رتبة", value: "add" }, { name: "➖ إزالة رتبة", value: "remove" }, { name: "📋 عرض الرتب", value: "list" }, { name: "🗑️ مسح الكل", value: "clear" }))
    .addRoleOption((o) => o.setName("role").setDescription("الرتبة (مطلوبة للإضافة والإزالة)").setRequired(false)))
    .addSubcommand((s) => s.setName("log_channel").setDescription("📋 تعيين قناة السجلات")
    .addChannelOption((o) => o.setName("channel").setDescription("القناة").addChannelTypes(discord_js_1.ChannelType.GuildText).setRequired(true)))
...
async function execute(interaction) {
    ...
    if (sub === "setup") {
        ...
        await interaction.reply({
            embeds: [
                new discord_js_1.EmbedBuilder()
                    .setColor(done >= 3 ? embeds_js_1.COLOR.green : done >= 2 ? embeds_js_1.COLOR.orange : embeds_js_1.COLOR.red)
                    ...
                    .addFields({ name: "🔢 إجمالي التكتات", value: `${config.ticketCounter ?? 0}`, inline: true }, { name: "🛡️ عدد رتب الدعم", value: `${config.supportRoleIds.length}`, inline: true }, { name: "🔐 نظام القنوات المزدوجة", value: config.adminCategoryId ? "✅ مفعّل" : "❌ معطّل", inline: true }, { name: "📌 للمساعدة", value: "`/helpt`", inline: true })
        ...
        if (sendNow) {
            ...
            await tc?.send({ embeds: [(0, embeds_js_1.panelEmbed)()], components: [(0, embeds_js_1.panelMenu)()] });
        }
        await interaction.reply({ embeds: [(0, embeds_js_1.successEmbed)(`قناة البنل: <#${ch.id}>${sendNow ? "\n✅ تم إرسال البنل فيها." : ""}`)], ephemeral: true });
        // ── Ticket Category ────────────────────────────────────────────────────────
    }
        ...
        (0, db_js_1.saveGuildConfig)(config);
        await interaction.reply({ embeds: [(0, embeds_js_1.successEmbed)(`فئة تكتات الأعضاء: **${ch.name}**`)], ephemeral: true });
        ...
        if (disable) {
            ...
            (0, db_js_1.saveGuildConfig)(config);
            await interaction.reply({ embeds: [(0, embeds_js_1.successEmbed)("✅ تم تعطيل نظام القنوات المزدوجة.")], ephemeral: true });
            return;
        }
        ...
        if (!ch) {
            ...
            await interaction.reply({ embeds: [new discord_js_1.EmbedBuilder().setColor(embeds_js_1.COLOR.purple).setTitle("🔐 نظام القنوات المزدوجة").setDescription([
                        "**كيف يعمل؟**",
        ...
        (0, db_js_1.saveGuildConfig)(config);
        await interaction.reply({
            embeds: [(0, embeds_js_1.successEmbed)([
                    `🔐 فئة الإدارة: **${ch.name}**`,
                    "",
        ...
        if (action === "list") {
            ...
            if (!roles.length) {
                await interaction.reply({ embeds: [new discord_js_1.EmbedBuilder().setColor(embeds_js_1.COLOR.orange).setTitle("🛡️ رتب فريق الدعم").setDescription("❌ لا توجد رتب مضافة حتى الآن.\nاستخدم `/configt support_role action:إضافة role:[الرتبة]`")], ephemeral: true });
                return;
            }
            const rows = roles.map((id, i) => `${i + 1}. <@&${id}> \`(${id})\``);
            await interaction.reply({
                embeds: [new discord_js_1.EmbedBuilder().setColor(embeds_js_1.COLOR.blue).setTitle(`🛡️ رتب فريق الدعم — ${roles.length}`).setDescription(rows.join("\n")).setFooter({ text: "FX9 • Support Roles" }).setTimestamp()],
                ephemeral: true,
            });
        }
        else if (action === "add") {
            if (!role) {
                await interaction.reply({ embeds: [(0, embeds_js_1.errorEmbed)("يجب تحديد رتبة للإضافة.")], ephemeral: true });
                return;
            }
            if (config.supportRoleIds.includes(role.id)) {
                await interaction.reply({ embeds: [(0, embeds_js_1.errorEmbed)(`<@&${role.id}> مضافة بالفعل.`)], ephemeral: true });
                return;
            }
            config.supportRoleIds.push(role.id);
            (0, db_js_1.saveGuildConfig)(config);
            await interaction.reply({ embeds: [(0, embeds_js_1.successEmbed)(`✅ تمت إضافة <@&${role.id}> لرتب فريق الدعم.\n**المجموع: ${config.supportRoleIds.length} رتبة**`)], ephemeral: true });
        }
        else if (action === "remove") {
            if (!role) {
                await interaction.reply({ embeds: [(0, embeds_js_1.errorEmbed)("يجب تحديد رتبة للإزالة.")], ephemeral: true });
                return;
            }
            if (!config.supportRoleIds.includes(role.id)) {
                await interaction.reply({ embeds: [(0, embeds_js_1.errorEmbed)(`<@&${role.id}> غير موجودة في القائمة.`)], ephemeral: true });
                return;
            }
            config.supportRoleIds = config.supportRoleIds.filter((id) => id !== role.id);
            (0, db_js_1.saveGuildConfig)(config);
            await interaction.reply({ embeds: [(0, embeds_js_1.successEmbed)(`✅ تمت إزالة <@&${role.id}>.\n**المتبقية: ${config.supportRoleIds.length} رتبة**`)], ephemeral: true });
        }
        else if (action === "clear") {
            const member = await interaction.guild.members.fetch(interaction.user.id);
            if (!member.permissions.has(discord_js_1.PermissionsBitField.Flags.Administrator)) {
                await interaction.reply({ embeds: [(0, embeds_js_1.errorEmbed)("يتطلب صلاحية **Administrator**.")], ephemeral: true });
                return;
            }
            const count = config.supportRoleIds.length;
            config.supportRoleIds = [];
            (0, db_js_1.saveGuildConfig)(config);
            await interaction.reply({ embeds: [(0, embeds_js_1.successEmbed)(`✅ تم مسح جميع الرتب (${count} رتبة).`)], ephemeral: true });
        }
        // ── Log Channel ────────────────────────────────────────────────────────────
    }
        ...
        (0, db_js_1.saveGuildConfig)(config);
        await interaction.reply({ embeds: [(0, embeds_js_1.successEmbed)(`قناة السجلات: <#${ch.id}>`)], ephemeral: true });
        // ── Show ──────────────────────────────────────────────────────────────────
    }
    else if (sub === "show") {
        ...
        await interaction.reply({
            embeds: [
                new discord_js_1.EmbedBuilder()
                    ...
                    .setDescription("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
                    .addFields({ name: "📣 قناة البنل", value: config.panelChannelId ? `<#${config.panelChannelId}>` : "❌", inline: true }, { name: "📁 فئة تكتات الأعضاء", value: config.ticketCategoryId ? `<#${config.ticketCategoryId}>` : "❌", inline: true }, { name: "🔐 فئة قنوات الإدارة", value: config.adminCategoryId ? `<#${config.adminCategoryId}>` : "❌ معطّل", inline: true }, { name: "📋 قناة السجلات", value: config.logChannelId ? `<#${config.logChannelId}>` : "❌", inline: true }, { name: "🔢 عداد التكتات", value: `${config.ticketCounter ?? 0}`, inline: true }, { name: "🟢 حالة النظام", value: config.ticketCategoryId && config.supportRoleIds.length > 0 ? "✅ جاهز" : "⚠️ يحتاج إعداداً", inline: true }, { name: "🔐 نظام الريلاي المزدوج", value: config.adminCategoryId ? "✅ مفعّل" : "❌ معطّل", inline: true }, { name: `🛡️ رتب الدعم (${config.supportRoleIds.length})`, value: rolesText }, { name: "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n💡 أوامر مفيدة", value: [
        ...
        const member = await interaction.guild.members.fetch(interaction.user.id);
        if (!member.permissions.has(discord_js_1.PermissionsBitField.Flags.Administrator)) {
            await interaction.reply({ embeds: [(0, embeds_js_1.errorEmbed)("يلزم صلاحية **Administrator**.")], ephemeral: true });
            return;
        }
        ...
        (0, db_js_1.saveGuildConfig)(config);
        await interaction.reply({
            embeds: [new discord_js_1.EmbedBuilder().setColor(embeds_js_1.COLOR.red).setTitle("🔄 تم مسح الإعدادات").setDescription("تم مسح جميع الإعدادات.\nاستخدم `/configt setup` لإعادة الإعداد.").setFooter({ text: `بواسطة: ${interaction.user.username}` }).setTimestamp()],
            ephemeral: true,
        });
    }
}
